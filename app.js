...
const APP_KEY = 'jsinfocomix_esp_state_v16';
const OLD_KEYS = ['jsinfocomix_esp_state_v15', 'jsinfocomix_esp_state_v14', 'jsinfocomix_esp_state_v13', 'jsinfocomix_esp_state_v12', 'jsinfocomix_esp_state_v1', 'jsinfocomix_esp_state_v2'];
const ADMIN_ID = 'admin';
const ADMIN_PIN = 'JS2026';
const DB_NAME = 'JSInfoComixESPRecordingsV16';
const DB_VERSION = 1;
...
const REAL_COMIC_SERIES_IMAGES = [
  'assets/comic-series-1.png',
  'assets/comic-series-2.png',
  'assets/comic-series-3.png',
  'assets/comic-series-4.png'
];
const REAL_COMIC_PANEL_IMAGES = [
  'assets/comic-panel-1.png',
  'assets/comic-panel-2.png',
  'assets/comic-panel-3.png',
  'assets/comic-panel-4.png'
];
function seriesThumbAsset(ep, series) {
  return REAL_COMIC_SERIES_IMAGES[(Number(series?.id || 1) - 1) % REAL_COMIC_SERIES_IMAGES.length];
}
function panelAsset(ep, series, index) {
  return REAL_COMIC_PANEL_IMAGES[(Number(index || 0)) % REAL_COMIC_PANEL_IMAGES.length];
}
...
function renderMiniSeriesThumb(ep, series, active = false) {
  return `<div class="series-thumb ${active ? 'active' : ''}">
    <div class="series-art-strip single">
      <img class="real-series-thumb" src="${seriesThumbAsset(ep, series)}" alt="Series ${series.id} comic preview" />
    </div>
...
function renderWeekThumbnail(ep) {
  return `<div class="week-thumb-real" role="img" aria-label="Week ${ep.week} comic thumbnail">
    <img src="${seriesThumbAsset(ep, { id: ((ep.week - 1) % 4) + 1 })}" alt="Week ${ep.week} comic preview" />
    <span class="week-thumb-badge">W${ep.week}</span>
  </div>`;
}
...
function renderPanelIllustration(panel, index, ep, series) {
  const src = panelAsset(ep, series, index);
  return `<div class="panel-art panel-art-frame" role="img" aria-label="Illustrated comic panel ${index + 1} for Week ${ep.week}">
    <img class="real-comic-art" src="${src}" alt="Comic panel ${index + 1}" />
    <div class="panel-badge">Series ${series?.id || 1} · Panel ${index + 1}</div>
    <div class="scene-badge">${sanitize(ep.scene)}</div>
  </div>`;
}
  ...
  return `
    <section class="episode-layout expanded-series-layout">
      <aside class="card episode-picker-card">
        ...
        <p class="mini">All 16 weeks are active in this course. Each week contains four real-comic style story series. Students must listen, choose a dialogue mission, record a response, and complete the quiz before the week is counted as completed.</p><div class="all-weeks-note">Weeks 1-16 are available. Scroll the week list to continue to Weeks 7-16.</div>
      ...
      <main class="grid">
        ...
        <div class="mission-banner series-mission-banner">
          <div class="card mission-card">
            ...
            <div class="quest-mini-row"><span>Listen Dialogue</span><span>Choose Character</span><span>Record Voice Mission</span><span>Submit Quiz</span><span>Unlock Boss XP</span></div><div class="interactive-activity-strip"><button class="ghost-btn" data-dialogue-as="Arka" data-dialogue-prompt="Continue the current story as Arka and give one professional response.">Interactive Activity: Continue as Arka</button><button class="ghost-btn" data-dialogue-as="Naya" data-dialogue-prompt="Respond to the current series as Naya and explain your idea clearly.">Interactive Activity: Continue as Naya</button><button class="ghost-btn" data-go="assessment" data-go-week="${ep.week}">Interactive Activity: Open Quiz Mission</button></div>
  ...
  return `<article class="comic-panel illustrated-panel premium-panel" data-panel="${index}">
    ...
    <div class="panel-actions">
      ...
      <div>
        ...
        <button class="primary-btn" data-practice-line="${sanitize(panel.line)}" data-practice-speaker="${sanitize(panel.speaker)}">Practice</button><button class="ghost-btn" data-record-panel="${sanitize(panel.line)}" data-record-speaker="${sanitize(panel.speaker)}">Record as ${sanitize(panel.speaker)}</button>
      </div>
    </div>
  </article>`;
}
...
function bindEpisodes() {
  ...
  $$('[data-record-panel]').forEach(button => button.addEventListener('click', event => {
    event.stopPropagation();
    const transcript = $('#epTranscript');
    const speaker = button.dataset.recordSpeaker || 'the character';
    const line = button.dataset.recordPanel || '';
    if (transcript) transcript.value = `Speaking as ${speaker}: ${line}\nMy extended response: `;
    showToast(`Recorder prepared for ${speaker}.`);
    transcript?.focus();
  }));
