...
const COURSE = [
  {
    ...
    panels: [
      ['Dr. J', 'Welcome to NusaTech Lab. Today you join an international software team.'],
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'Use this structure: problem, condition, evidence, and solution.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'Professional UI/UX English explains what users see and why it matters.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'Exactly. Explain database relationships with clear examples.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'Good technical English makes complex instructions easier to understand.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'Use careful language: detected, suspected, evidence, recommendation.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'A strong progress report is specific, brief, and solution-oriented.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'Boss Battle: communicate professionally under pressure.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'Use data language: increase, decrease, trend, comparison, implication.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'Professional support language balances empathy and responsibility.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'Good AI literacy means explaining benefits, risks, and safeguards.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'Testing English uses objective and precise reporting.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'Persuasive ESP connects technical features to real user needs.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'International communication values clarity, politeness, and realistic commitment.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'A final script helps you sound professional and confident.']
    ],
    quiz: [
  ...
  },
  {
    ...
    panels: [
      ...
      ['Dr. J', 'Your final mission is to show professional communication growth.'],
...
const DEFAULT_STATE = {
  ...
  classMeta: {
    ...
    semester: '16 Weeks',
    institution: 'Cipta Wacana University',
    lecturer: 'Dr. Joko Slamet'
  },
  submissions: [],
  settings: {
...
function loadState() {
  try {
    ...
    return { ...clone(DEFAULT_STATE), ...parsed, users: parsed.users || {}, submissions: parsed.submissions || [] };
  } catch (e) {
    console.warn(e);
...
function setupAuth() {
  ...
  $('#registerForm').addEventListener('submit', e => {
    ...
    const id = uid('user');
    state.users[id] = {
      id, role: 'student', name: $('#regName').value.trim(), email,
      className: $('#regClass').value.trim(), pin: $('#regPin').value,
      createdAt: new Date().toISOString(), progress: clone(DEFAULT_STATE.users.progress || {})
    };
  ...
  $('#lecturerForm').addEventListener('submit', e => {
    ...
    const existingLecturer = Object.values(state.users).find(u => u.role === 'lecturer' && u.email.toLowerCase() === email);
    if (existingLecturer) return authError('A lecturer workspace with this email or ID already exists. Use Login instead.');
    const id = uid('lecturer');
    state.users[id] = {
      id, role: 'lecturer', name: $('#lecName').value.trim(), email,
    ...
    state.classMeta.lecturer = state.users[id].name;
    state.classMeta.institution = state.users[id].institution;
    state.currentUserId = id;
    saveState();
    $('#authOverlay').classList.add('hidden');
    renderApp();
    toast('Lecturer workspace created.');
  });
  ...
  $('#loginForm').addEventListener('submit', e => {
    ...
    const user = Object.values(state.users).find(u => u.email.toLowerCase() === login || u.name.toLowerCase() === login || u.id === login);
    if (!user || user.pin !== pin) return authError('Login failed. Check the email/ID and PIN saved in this browser.');
  ...
  $('#demoSeedBtn').addEventListener('click', () => {
    seedDemo();
    toast('Demo class data created. Use the demo accounts shown in Settings after login.');
  });
}
...
function seedDemo() {
  ...
  state.users[lecturerId] = {
    id: lecturerId, role: 'lecturer', name: 'Dr. Joko Slamet', email: 'lecturer@demo.local', institution: 'Cipta Wacana University', pin: '2026', createdAt: new Date().toISOString(), progress: { xp: 0, completedWeeks: {}, quizzes: {}, voiceIds: [], notes: {}, words: {}, reflections: {} }
  };
  ...
  names.forEach((name, i) => {
    ...
    state.users[id] = {
      id, role: 'student', name, email: `student${i+1}@demo.local`, className: 'Informatics A', pin: '1234', createdAt: new Date().toISOString(),
...
function renderNav() {
  const nav = $('#navList');
  nav.innerHTML = NAV_ITEMS.map(([id, icon, label]) => `<button class="nav-btn" data-view="${id}"><span class="nav-icon">${icon}</span><span>${label}</span></button>`).join('');
...
function renderApp() {
  ...
  $('#userPill').innerHTML = user ? `${sanitize(user.name)} · ${user.role === 'lecturer' ? 'Lecturer' : `Level ${levelFromXp(userProgress(user).xp)}`}` : 'Guest';
  if (!user) return;
  const root = $('#viewRoot');
  const routes = {
    home: renderHome,
  ...
  return `
    ...
    <section class="grid three">
      ...
      <div class="card">
        ...
        <p><strong>${sanitize(state.classMeta.courseTitle)}</strong><br>${sanitize(state.classMeta.semester)} · ${sanitize(state.classMeta.institution)}<br>Lecturer: ${sanitize(state.classMeta.lecturer)}</p>
      </div>
    </section>
  ...
  return `
    <section class="grid two">
      <div class="card">
        ...
        ${user.role === 'student' ? `<label>Class <input id="setClass" value="${sanitize(user.className || '')}"></label>` : `<label>Institution <input id="setInstitution" value="${sanitize(user.institution || state.classMeta.institution)}"></label>`}
    ...
    ${user.role === 'lecturer' ? `<section class="card">
      <h3>Class Account Overview</h3>
      <p>This lecturer-only overview lists accounts saved in this browser for class management. PIN values are never displayed.</p>
      <div class="table-wrap"><table class="data-table"><thead><tr><th>Role</th><th>Name</th><th>Email/ID</th><th>Class/Institution</th></tr></thead><tbody>${Object.values(state.users).map(u => `<tr><td>${u.role}</td><td>${sanitize(u.name)}</td><td>${sanitize(u.email)}</td><td>${sanitize(u.className || u.institution || '-')}</td></tr>`).join('')}</tbody></table></div>
...
function exportAllData() {
  ...
  const payload = isLecturer
    ? { app: 'JSInfoComix ESP', version: '1.0.0', type: 'full-class-backup', exportedAt: new Date().toISOString(), state }
    : {
        app: 'JSInfoComix ESP', version: '1.0.0', type: 'student-portfolio-backup', exportedAt: new Date().toISOString(),
