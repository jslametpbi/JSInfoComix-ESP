...
const APP_KEY = 'jsinfocomix_esp_state_v12';
const OLD_KEYS = ['jsinfocomix_esp_state_v1', 'jsinfocomix_esp_state_v2'];
const ADMIN_ID = 'admin';
const ADMIN_PIN = 'JS2026';
const DB_NAME = 'JSInfoComixESPRecordingsV12';
const DB_VERSION = 1;
...
function roleName(user = currentUser()) {
  ...
  if (user.role === 'lecturer') return user.status === 'approved' ? 'Verified Lecturer' : 'Pending Lecturer';
...
function bindAuth() {
  ...
  $('#loginForm').addEventListener('submit', event => {
    ...
    if (role === 'admin') ensureAdminUser();
    const user = Object.values(state.users).find(item => item.role === role && String(item.email).toLowerCase() === identity.toLowerCase() && item.pin === pin);
  ...
  $('#demoSeedBtn').addEventListener('click', seedDemoClass);
}
  ...
  return `
    <section class="episode-layout">
      <aside class="card">
        ...
        <p class="mini">Click a week to open the real interactive comic episode.</p>
        <div class="week-list">${COURSE.map(item => `<button class="week-btn ${item.week === ep.week ? 'active' : ''}" type="button" data-week="${item.week}"><strong>Week ${item.week}<span>${p.completedWeeks[item.week] ? '✓' : ''}</span></strong><span>${sanitize(item.title)}</span><span>${sanitize(item.focus)}</span></button>`).join('')}</div>
      </aside>
      <main class="grid">
        <div class="card">
          <div class="comic-top">
            <div>
              <p class="eyebrow">Week ${ep.week} · ${sanitize(ep.scene)}</p>
              <h2>${sanitize(ep.title)}</h2>
              <p><strong>ESP Focus:</strong> ${sanitize(ep.focus)}</p>
            </div>
            <div class="hero-actions">
              <button class="ghost-btn" data-speak="${sanitize(ep.panels.map(p => `${p.speaker}: ${p.line}`).join(' '))}">▶ Listen Full Episode</button>
              <button class="success-btn" id="markEpisodeDone">Mark Complete</button>
            </div>
          </div>
          <div class="comic-stage">
            ${ep.panels.map((panel, index) => renderComicPanel(panel, index, ep)).join('')}
          </div>
        </div>
        <div class="mission-banner">
          <div class="card">
            ...
            <p>Students listen to the comic dialogue, study the expressions, record their spoken response, receive local AI-style feedback, and complete the weekly assessment.</p>
            <div class="chip-list">${ep.terms.map(term => `<span class="chip">${sanitize(term)}</span>`).join('')}</div>
            <div class="hero-actions">
              <button class="primary-btn" data-go="assessment" data-go-week="${ep.week}">Open Weekly Quiz</button>
          ...
          ${renderRecorder('ep', ep.week, 'Episode Voice Mission')}
        </div>
        <section class="grid two">
          ...
          <div class="card">
            ...
            <label>What professional expression did you learn from this episode?
...
function renderComicPanel(panel, index, ep) {
  return `<article class="comic-panel" data-panel="${index}">
    <div class="comic-bg"></div>
    <div class="character">
      <div class="avatar-comic ${sanitize(panel.role)}" aria-hidden="true"></div>
      <div class="speech"><strong>${sanitize(panel.speaker)}</strong>${sanitize(panel.line)}</div>
    </div>
    <div class="comic-scene" aria-hidden="true"></div>
    <div class="panel-actions">
      ...
      <div>
        ...
        <button class="primary-btn" data-practice-line="${sanitize(panel.line)}">Practice</button>
      </div>
    </div>
  </article>`;
}
...
function renderAssessment() {
  ...
  return `<section class="grid">
    <div class="card">
      <div class="comic-top"><div><p class="eyebrow">Assessment Missions</p><h2>Weekly Formative Quiz</h2><p>Complete the quiz after listening to and practicing the comic episode.</p></div><select id="quizWeekSelect">${COURSE.map(item => `<option value="${item.week}" ${item.week === ep.week ? 'selected' : ''}>Week ${item.week} · ${sanitize(item.title)}</option>`).join('')}</select></div>
      <h3>Week ${ep.week}: ${sanitize(ep.title)}</h3>
      <div id="quizBox">${ep.quiz.map((q, qi) => `<div class="quiz-question"><strong>${qi + 1}. ${sanitize(q.q)}</strong><div class="option-list">${q.options.map((option, oi) => `<button class="option" type="button" data-q="${qi}" data-o="${oi}">${sanitize(option)}</button>`).join('')}</div></div>`).join('')}</div>
      <div class="hero-actions"><button class="primary-btn" id="submitQuizBtn">Submit Quiz</button><button class="ghost-btn" data-go="episodes" data-go-week="${ep.week}">Back to Comic</button></div>
    ...
    <section class="grid two">
      <div class="card"><h3>Formative Assessment 60%</h3><div class="table-wrap"><table class="data-table"><tbody><tr><td>Weekly comic missions</td><td>20%</td></tr><tr><td>Vocabulary and reading tasks</td><td>10%</td></tr><tr><td>Voice portfolio</td><td>15%</td></tr><tr><td>Peer feedback and collaboration</td><td>10%</td></tr><tr><td>Reflective journal</td><td>5%</td></tr></tbody></table></div></div>
      <div class="card"><h3>Summative Assessment 40%</h3><div class="table-wrap"><table class="data-table"><tbody><tr><td>Midterm tech-meeting simulation</td><td>15%</td></tr><tr><td>Final product pitch</td><td>15%</td></tr><tr><td>Final ESP comic-based test</td><td>10%</td></tr></tbody></table></div></div>
    </section>
  </section>`;
}
...
function renderAdmin() {
  ...
  return `<section class="grid">
    ...
    <div class="grid three">
      ...
      <div class="card"><h3>Verified Lecturers</h3><p><strong>${allLecturers().filter(l => l.status === 'approved').length}</strong> approved lecturers</p></div>
    </div>
  </section>`;
}
...
function bindEpisodes() {
  ...
  $$('[data-practice-line]').forEach(button => button.addEventListener('click', event => {
    ...
    if (transcript) transcript.value = `I will respond to this line: ${button.dataset.practiceLine}\n`;
    showToast('Practice prompt copied to the episode recorder.');
    transcript?.focus();
  }));
