...
const APP_KEY = 'jsinfocomix_esp_state_v14';
const OLD_KEYS = ['jsinfocomix_esp_state_v13', 'jsinfocomix_esp_state_v12', 'jsinfocomix_esp_state_v1', 'jsinfocomix_esp_state_v2'];
const ADMIN_ID = 'admin';
const ADMIN_PIN = 'JS2026';
const DB_NAME = 'JSInfoComixESPRecordingsV14';
const DB_VERSION = 1;
...
function renderMiniCharacterGroup(names, t, offset = 0) {
  const clean = (names || ['Arka']).slice(0, 4);
  return `<svg viewBox="0 0 160 72" class="mini-group" aria-hidden="true">
    <rect width="160" height="72" rx="12" fill="${t.wash}"/>
    <path d="M0 58 C45 37 72 69 160 42 V72 H0 Z" fill="${t.primary}" opacity=".13"/>
    ${clean.map((name, i) => {
      const x = 24 + i * 35;
      const c = characterColor(name, t.primary);
      const hair = CHARACTER_PROFILES[name]?.hair || '#111827';
      const female = CHARACTER_PROFILES[name]?.role === 'female';
      const mentor = name === 'Dr. Joe';
      return `<g transform="translate(${x} 42)"><rect x="-13" y="7" width="26" height="26" rx="8" fill="${c}"/><circle cx="0" cy="-6" r="13" fill="#f7c59f"/><path d="M-13-8 C-10-25 10-25 13-9 ${female ? 'C8-18 -8-18 -13-8' : ''}" fill="${hair}"/>${mentor ? '<circle cx="-4" cy="-5" r="4" fill="none" stroke="#111827"/><circle cx="5" cy="-5" r="4" fill="none" stroke="#111827"/>' : ''}<circle cx="-4" cy="-5" r="1.3"/><circle cx="5" cy="-5" r="1.3"/></g>`;
    }).join('')}
  </svg>`;
}
...
function renderCharacterAvatar(name, x, y, scale = 1, flip = 1) {
  ...
  if (isBot) {
    return `<g transform="translate(${x} ${y}) scale(${scale * flip} ${scale})"><rect x="-24" y="-49" width="48" height="39" rx="12" fill="#ccfbf1" stroke="#0f766e" stroke-width="4"/><circle cx="-9" cy="-30" r="4" fill="#0f766e"/><circle cx="9" cy="-30" r="4" fill="#0f766e"/><path d="M-13-17h26" stroke="#0f766e" stroke-width="4" stroke-linecap="round"/><rect x="-31" y="-9" width="62" height="61" rx="18" fill="${profile.color}"/></g>`;
  }
  return `<g transform="translate(${x} ${y}) scale(${scale * flip} ${scale})">
    <path d="M-35 54 C-31 14 31 14 35 54 Z" fill="${profile.color}"/>
    <path d="M-17 16 L0 43 L17 16" fill="${profile.shirt}" opacity=".95"/>
    <circle cx="0" cy="-18" r="27" fill="#f7c59f" stroke="#1f2937" stroke-width="2"/>
    <path d="M-28-20 C-26-52 19-52 29-22 C18-34-11-36-28-20 Z" fill="${profile.hair}"/>
    ${isFemale ? '<path d="M-30-13 C-47 8-34 34-18 44 C-22 18-18 0-8-6 Z" fill="#24111f"/><path d="M28-12 C45 8 31 34 16 44 C20 17 17 0 8-6 Z" fill="#24111f"/>' : ''}
    <circle cx="-9" cy="-16" r="3" fill="#111827"/><circle cx="9" cy="-16" r="3" fill="#111827"/>
    ${isMentor ? '<circle cx="-9" cy="-16" r="8" fill="none" stroke="#111827" stroke-width="2"/><circle cx="9" cy="-16" r="8" fill="none" stroke="#111827" stroke-width="2"/><path d="M-1-16h2" stroke="#111827" stroke-width="2"/>' : ''}
    <path d="M-9 4 C-3 9 4 9 10 4" stroke="#111827" stroke-width="3" fill="none" stroke-linecap="round"/>
  </g>`;
}

function renderCharacterGroup(panel, index, t) {
  const cast = (panel.cast?.length ? panel.cast : [panel.speaker]).slice(0, 6);
  const positions = cast.length <= 2 ? [[88,125,1.15,1],[272,125,1.15,-1]] :
    cast.length <= 4 ? [[58,128,.9,1],[138,123,1,1],[222,123,1,-1],[304,128,.9,-1]] :
    [[41,131,.76,1],[98,126,.86,1],[157,122,.9,1],[218,122,.9,-1],[279,126,.86,-1],[330,131,.76,-1]];
  return cast.map((name, i) => renderCharacterAvatar(name, ...(positions[i] || [70 + i*52, 126, .8, 1]))).join('');
}
...
function renderPanelIllustration(panel, index, ep, series) {
  ...
  return `<svg class="panel-art" viewBox="0 0 460 238" role="img" aria-label="Illustrated comic panel ${index + 1} for Week ${ep.week}">
    <defs>
      <linearGradient id="${id}" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="${t.wash}"/><stop offset="1" stop-color="#ffffff"/></linearGradient>
      <pattern id="grid_${ep.week}_${series?.id || 1}_${index}" width="18" height="18" patternUnits="userSpaceOnUse"><path d="M18 0H0V18" fill="none" stroke="${t.primary}" stroke-width="1" opacity=".10"/></pattern>
    </defs>
    <rect width="460" height="238" rx="25" fill="url(#${id})"/>
    <rect width="460" height="238" rx="25" fill="url(#grid_${ep.week}_${series?.id || 1}_${index})"/>
    <path d="M0 196 C80 150 134 223 230 174 C322 127 381 164 460 118 V238 H0 Z" fill="${t.primary}" opacity=".12"/>
    <rect x="18" y="20" width="424" height="138" rx="18" fill="#fff" opacity=".43"/>
    ${renderSceneObject(ep.week, t, false)}
    ${renderCharacterGroup(panel, index, t)}
    <path d="M28 25 h182 q16 0 16 16 v52 q0 16-16 16 h-83 l-22 20 5-20 H28 q-16 0-16-16 V41 q0-16 16-16z" fill="#ffffff" stroke="#d1dbe8" stroke-width="2"/>
    <text x="34" y="48" font-size="15" font-weight="900" fill="#0f172a">${sanitize(speaker)}</text>
    <foreignObject x="34" y="56" width="170" height="47"><div xmlns="http://www.w3.org/1999/xhtml" style="font-family:Inter,Arial,sans-serif;font-size:13px;line-height:1.18;color:#0f172a;">${sanitize(panel.line).slice(0, 128)}</div></foreignObject>
    <rect x="18" y="202" width="112" height="24" rx="12" fill="#0f172a" opacity=".88"/>
    <text x="74" y="219" text-anchor="middle" font-size="12" fill="#fff" font-weight="800">Series ${series?.id || 1} · Panel ${index + 1}</text>
  </svg>`;
}
  ...
  return `
    <section class="episode-layout expanded-series-layout">
      <aside class="card episode-picker-card">
        ...
        <p class="mini">Each week now contains 4 related comic series. Open a week, select a series, listen to the dialogue, then record your response as a character.</p>
        ...
        <div class="card comic-episode-card premium-series-card">
          <div class="comic-top">
            ...
            <div class="hero-actions">
              ...
              <button class="success-btn" id="markEpisodeDone">Mark Complete</button>
            </div>
          </div>
        ...
        <div class="mission-banner series-mission-banner">
          <div class="card mission-card">
            ...
            <div class="quest-mini-row">
              <span>+60 XP Complete Episode</span><span>+100 XP Quiz Pass</span><span>+145 XP Voice Mission</span><span>+${active.xp} XP Series</span>
            </div>
            <div class="hero-actions">
...
function renderVocabulary() {
  ...
  return `<section class="grid">
    <div class="card">
      <div class="comic-top"><div><p class="eyebrow">Week ${ep.week}</p><h2>Vocabulary Lab: ${sanitize(ep.title)}</h2><p>${sanitize(ep.focus)}</p></div><select id="vocabWeek">${COURSE.map(item => `<option value="${item.week}" ${item.week === ep.week ? 'selected' : ''}>Week ${item.week} · ${sanitize(item.title)}</option>`).join('')}</select></div>
...
function renderCoach() {
  ...
  return `<section class="grid two">
    <div class="card">
      ...
      <button class="primary-btn" id="analyzeTextBtn">Analyze My Speaking</button>
    </div>
  ...
  return `<section class="grid">
    <div class="assessment-hero">
      <div>
        ...
        <p>Each weekly assessment now contains five mission questions, XP rewards, a voice requirement, and a boss challenge to make formative assessment more engaging.</p>
      </div>
    ...
    <section class="grid two assessment-grid">
      ...
      <div class="grid">
        ${renderQuestBoard(ep)}
        <div class="card reward-card"><h3>Reward System</h3><div class="reward-row"><span>Comic completed</span><strong>+60 XP</strong></div><div class="reward-row"><span>Quiz mission passed</span><strong>+70 to +170 XP</strong></div><div class="reward-row"><span>Voice portfolio saved</span><strong>+120 XP+</strong></div><div class="reward-row"><span>Boss challenge</span><strong>+${ep.quest.xp} XP</strong></div></div>
      </div>
    </section>
    <section class="grid two">
...
function bindEpisodes() {
  ...
  $('#markEpisodeDone')?.addEventListener('click', () => {
    const p = progressOf();
    if (!p.completedWeeks[state.selectedWeek]) addXp(60);
    p.completedWeeks[state.selectedWeek] = true;
    saveState();
    renderApp();
    showToast(`Week ${state.selectedWeek} marked complete.`);
  });
...
function bindAssessment() {
  ...
  $('#submitQuizBtn')?.addEventListener('click', () => {
    ...
    if (score >= 70) {
      progressOf().completedWeeks[ep.week] = true;
    ...
    saveState();
    $('#quizResult').innerHTML = `<div class="feedback-card"><strong>Quiz score:</strong> ${score}/100 · ${correct}/${ep.quiz.length} correct. ${score >= 70 ? 'Mission passed and XP added.' : 'Review the comic and try again.'}</div>`;
...
function bindRecorder(prefix, getWeek) {
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    progressOf(user).completedWeeks[week] = true;
    addXp(120 + Math.round(feedback.score / 4), user);
    saveState();
    showToast('Voice recording saved to portfolio.');
    setView('portfolio');
  });
}
