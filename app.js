...
const APP_KEY = 'jsinfocomix_esp_state_v13';
const OLD_KEYS = ['jsinfocomix_esp_state_v12', 'jsinfocomix_esp_state_v1', 'jsinfocomix_esp_state_v2'];
const ADMIN_ID = 'admin';
const ADMIN_PIN = 'JS2026';
const DB_NAME = 'JSInfoComixESPRecordingsV13';
const DB_VERSION = 1;
...
function renderPanelIllustration(panel, index, ep) {
  const t = sceneTheme(ep.week);
  const id = `comicGradient_${ep.week}_${index}`;
  return `<svg class="panel-art" viewBox="0 0 360 190" role="img" aria-label="Illustrated comic panel ${index + 1} for Week ${ep.week}">
    <defs>
      <linearGradient id="${id}" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="${t.wash}"/><stop offset="1" stop-color="#ffffff"/></linearGradient>
      <pattern id="dots_${ep.week}_${index}" width="12" height="12" patternUnits="userSpaceOnUse"><circle cx="2" cy="2" r="1" fill="${t.primary}" opacity=".12"/></pattern>
    </defs>
    <rect width="360" height="190" rx="24" fill="url(#${id})"/>
    <rect width="360" height="190" rx="24" fill="url(#dots_${ep.week}_${index})"/>
    <path d="M0 151 C78 121 128 183 206 145 C278 111 305 133 360 103 V190 H0 Z" fill="${t.primary}" opacity=".12"/>
    <rect x="15" y="14" width="330" height="117" rx="18" fill="#ffffff" opacity=".44"/>
    ${renderSceneObject(ep.week, t, false)}
    ${renderCharacterSvg(panel, index, t)}
    <rect x="17" y="153" width="70" height="22" rx="11" fill="#0f172a" opacity=".85"/>
    <text x="52" y="168" text-anchor="middle" font-size="12" fill="#fff" font-weight="800">Panel ${index + 1}</text>
  </svg>`;
}
  ...
  return `
    <section class="episode-layout">
      <aside class="card episode-picker-card">
        ...
        <p class="mini">Click a week to open the illustrated interactive comic episode. Each episode has scene art, listening, practice, recording, vocabulary, and quiz missions.</p>
        <div class="week-list">${COURSE.map(item => `<button class="week-btn illustrated ${item.week === ep.week ? 'active' : ''}" type="button" data-week="${item.week}"><div class="week-art">${renderWeekThumbnail(item)}</div><div class="week-copy"><strong>Week ${item.week}<span>${p.completedWeeks[item.week] ? '✓' : ''}</span></strong><span>${sanitize(item.title)}</span><small>${sanitize(item.focus)}</small></div></button>`).join('')}</div>
      </aside>
      <main class="grid">
        <div class="card comic-episode-card">
          <div class="comic-top">
            <div>
              <p class="eyebrow">Week ${ep.week} · Illustrated ${sanitize(ep.scene)} scene</p>
          ...
          <div class="comic-stage illustrated-comic-stage">
        ...
        <div class="mission-banner">
          <div class="card mission-card">
            ...
            <p>Students listen to the comic dialogue, study professional expressions, record a spoken response, receive local AI-style feedback, and unlock gamified assessment missions.</p>
            <div class="chip-list">${ep.terms.map(term => `<span class="chip">${sanitize(term)}</span>`).join('')}</div>
            <div class="quest-mini-row">
              <span>+60 XP Complete Episode</span><span>+100 XP Quiz Pass</span><span>+145 XP Voice Mission</span>
            </div>
            <div class="hero-actions">
...
function renderComicPanel(panel, index, ep) {
  return `<article class="comic-panel illustrated-panel" data-panel="${index}">
    <div class="panel-art-wrap">${renderPanelIllustration(panel, index, ep)}</div>
